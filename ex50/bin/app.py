...
def doorRoom(player): # locked door room
    ...
    if doorOpened and 4 in choices: # removes invalid choices
        choices.remove(4)
        
    if doorOpened and 2 in choices:
        choices.remove(2)
        
    if doorOpened and 6 not in choices:
        choices.append(6)
        
    if not locked and not doorOpened and 4 in choices:
        choices.remove(4)
        
    if 'key' in all_dropped or 'key' in player.backpack and 3 in choices:
        choices.remove(3)
    
    print ('\nYou come to a door surrounded in bones.')
    print ('There is a large handle made of a human femur.')
    print ('Next to the door lies a golden key.')
    print ('There is a keyhole.\n')
    
    while True: # keeps track of whether door is locked/open 
    
        if doorOpened and 4 in choices:
            choices.remove(4)
        ...
        if action.isdigit() and int(action) in choices: # catches invalid user inputs
            ...
            if action == 1:
                ...
                if key not in player.backpack: # True if user doesn't have item they want to put in keyhole
                    ...
                    if key == 'finger':
                        ...
                        print ('A surge of electricity flows through the key, and shocks you to death. Game Over!')
                        return None
            ...
            elif action == 6: # go through locked door
...
def printDoorChoices(choices):
    ...
    time.sleep(2) # allows the user to process what happened after their last action
    print('\nChoices: ')
    choicesText = ['1. turn around','2. open the door','3. pick up golden key', '4. place item in keyhole', '5. open backpack', '6. go through door'] # list of all possible choices
        ...
        if action.isdigit() and int(action) in choices: # catches invalid user inputs
            ...
            if action == 1: # leave
                ...
                if (len(player.backpack) > 0):
                    ...
                    if drop == 'torch':
                        print ('You cannot drop the torch right now.')
...
def chanceRoom(player):
    ...
    playedOnce = False # True if have player roulette game at least once
    
    choices = [1,2,3,4,5,6] # list of choices player can make
    ...
    while (action != 3): # Runs until user wants to leave the room (action 3)
        ...
        if action.isdigit() and int(action) in choices: # catches invalid user inputs
            ...
            if (action == 1 and (('red coin' in player.backpack and 'black coin' in player.backpack and 'green coin' in player.backpack) or playedOnce)): # gamble and True if all coins in backpack or already played once
                ...
                if (playedOnce == False):
                    player.backpack.remove('black coin') # remove coins from backpack only first time playing
...
def playRoulette(): # returns True if player wins, False otherwise
    ...
    while (guessRight < 3):
        ...
        elif (rGuess == 'b'): # True if user guessed black
            croupier(rNum) # animated roll
            print('\n')
        ...
        elif (rGuess == 'g'): # True if user guessed green
...
def start(player): # start room
    ...
    global timeallowed
    global t0
    choices = [1, 2, 3, 4] # list of choices player can make
    action = 0
    ...
    print('You hear a strange breathing sound behind you.\n')
    ...
    while True:
        ...
        if action.isdigit() and int(action) in choices: # catches invalid user input
            ...
            if action == 1: # north bedroom
                ...
                print ('You turn around and see a sign that reads:')
                print ('Shouldn\'t you be studying for midterms?')
                print ('Anxiety hits you and you have a heart attack, dying on the spot. Game Over!')
                return None
...
def printStartChoices(choices): # prints all actions that can be taken
    ...
    time.sleep(2) # allows the user to process what happened after their last action
    print('\nChoices: ')
    choicesText = ['1. go north','2. go west','3. go east', '4. look behind you'] # list of all possible choices
    
    for i in range(1,5): # loops through all possible actions
...
# 0 1 6 y 7 y 8 3 6 1 2 3 9 1 2 4 finger 2 6 2 y 1 (gamble)
# 3 1 6 y 8 3 6 1 3 2 9 1 1 10 torch 7 y 8 2 4 finger 2 6 2 y 1 (gamble) 1